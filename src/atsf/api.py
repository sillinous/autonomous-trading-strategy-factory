...
def create_app(registry: ExperimentRegistry | None = None) -> FastAPI:
    ...
    def run(run_id: str, store: ExperimentRegistry = Store) -> dict:
    ...
    @app.get("/runs/{run_id}/provenance-graph", dependencies=[Auth])
    def provenance_graph(run_id: str, store: ExperimentRegistry = Store) -> dict:
    ...
    @app.get("/runs/{run_id}/certificate", dependencies=[Auth])
    def get_certificate(run_id: str, store: ExperimentRegistry = Store) -> dict:
    ...
    @app.get("/runs/{run_id}/certificate/verify", dependencies=[Auth])
    def verify_certificate(run_id: str, store: ExperimentRegistry = Store) -> dict:
    ...
    @app.get("/runs/{run_id}/verify", dependencies=[Auth])
    def verify_run(run_id: str, store: ExperimentRegistry = Store) -> dict:
        try:
    ...
    def verify_replay(run_id: str, request: ReplayVerificationRequest, store: ExperimentRegistry = Store) -> dict:
        try:
            ...
            try:
                ...
                return {"run_id": run_id, "valid": False, "reason": f"replay data bundle is invalid: {exc}", "event_count": baseline.manifest.event_count, "ledger_fingerprint": baseline.manifest.ledger_fingerprint}
    ...
    @app.post("/runs/{run_id}/certificate", dependencies=[Auth])
    def certificate(run_id: str, request: ReplayVerificationRequest, store: ExperimentRegistry = Store) -> dict:
        try:
            frames = {sid: validate_market_data(pd.DataFrame([bar.model_dump() for bar in bars]).set_index("timestamp")) for sid, bars in request.data.items()}
            run = store.get_portfolio_run(run_id)
            if run is None:
    ...
    @app.post("/portfolios/{portfolio_id}/paper-runs", status_code=201, dependencies=[Auth])
    def paper_run(portfolio_id: str, request: PaperRunRequest, store: ExperimentRegistry = Store) -> dict:
        try:
            frames = {sid: validate_market_data(pd.DataFrame([bar.model_dump() for bar in bars]).set_index("timestamp")) for sid, bars in request.data.items()}
