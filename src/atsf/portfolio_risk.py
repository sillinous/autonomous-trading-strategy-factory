...
def build_risk_aware_portfolio(
    ...
    *,
    ...
    If the complete diversified selection is too risky, candidates are removed from
    the lowest-ranked end one at a time. This keeps the selection deterministic and
    prevents an over-risky portfolio from being admitted merely because individual
    strategies passed their own gates.
    """
    ...
    while candidates:
        ...
        volatility = _volatility(returns, allocation)
        if volatility <= policy.max_portfolio_volatility:
