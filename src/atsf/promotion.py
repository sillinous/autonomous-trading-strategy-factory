...
def research_to_paper(
    ...
    if policy.require_robustness:
        if robustness is None:
            reasons.append("execution robustness evidence is required")
        elif not robustness.passed:
            reasons.extend(robustness.reasons)
        else:
            baseline_equity = float(robustness.baseline.equity.iloc[-1])
            stressed_equities = [
                float(scenario.equity.iloc[-1]) for _, scenario in robustness.scenarios
            ]
    ...
    elif robustness is not None and not robustness.passed:
        reasons.extend(robustness.reasons)
