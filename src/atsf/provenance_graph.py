...
def build_research_provenance_graph(
    ...
    dataset_node = _node(
        ...
        f"{dataset_id}:{dataset_version}",
        {
            ...
            "timeframe": definition.get("data_timeframe"),
    ...
    nodes: dict[str, ProvenanceNode] = {
        root.key: root,
        f"portfolio:{portfolio_id}": portfolio_node,
        f"dataset:{dataset_id}:{dataset_version}": dataset_node,
        f"bundle:{bundle_version}": bundle_node,
    }
    edges: list[ProvenanceEdge] = [
        _edge(dataset_node, bundle_node, "bundles"),
        _edge(bundle_node, portfolio_node, "input_to"),
        _edge(portfolio_node, root, "executed_as"),
    ]
    ...
    manifest = _node(
        ...
        run_id,
        {
            "event_count": config.get("ledger_event_count"),
