        ...
        for event_payload in feedback_store.list_for_strategy(strategy_id):
            ...
            if event.research_request_id is not None:
                ...
                for row in successor_store.list_for_request(request.request_id):
                    ...
                    candidate_lineage_nodes, candidate_lineage_edges = _lineage_nodes(store, {candidate_strategy_id})
                    nodes.update({f"strategy:{key}": value for key, value in candidate_lineage_nodes.items() if not key.startswith("lineage:")})
                    nodes.update({key: value for key, value in candidate_lineage_nodes.items() if key.startswith("lineage:")})
                    normalized_edges.extend(candidate_lineage_edges)
                    strategy_node = nodes.get(f"strategy:{candidate_strategy_id}")
                    if strategy_node is None:
                        raise ValueError(f"successor strategy is absent: {candidate_id}")
                    normalized_edges.append(_edge(strategy_node, candidate_node, "defines_strategy"))
