...
class ExperimentRegistry:
    ...
    def _create_schema(self) -> None:
        self._connection.executescript(
            ...
            CREATE TABLE IF NOT EXISTS portfolios (
                portfolio_id TEXT PRIMARY KEY, definition_json TEXT NOT NULL
            );
    ...
    def _validate_portfolio_run_payload(self, run_id: str, portfolio_id: str, final_equity: float, attribution: list[dict[str, Any]], dataset_id: str, dataset_version: str, data_bundle_version: str, execution_fingerprint: str, execution_config: dict[str, Any], audit_events: list[PortfolioAuditEvent]) -> tuple[str, list[tuple[Any, ...]], list[tuple[Any, ...]]]:
        ...
        for item in attribution:
            ...
            if strategy not in portfolio_members or strategy in seen: raise ValueError("attribution must contain unique persisted portfolio members")
            if not all(math.isfinite(float(item[key])) for key in ("return_contribution", "risk_contribution")): raise ValueError("attribution values must be finite")
            seen.add(strategy); attribution_rows.append((run_id, strategy, float(item["return_contribution"]), float(item["risk_contribution"])))
        ...
        audit_rows: list[tuple[Any, ...]] = []; audit_ids: set[str] = set()
        for event in ordered:
            event_id = audit_event_id(event)
            if event_id in audit_ids or event.strategy_id not in portfolio_members: raise ValueError("audit events must be unique and reference persisted portfolio members")
            audit_ids.add(event_id); audit_rows.append((run_id, event_id, event.sequence, event.strategy_id, event.action, event.timestamp, event.quantity, event.price, event.fee))
    ...
    def save_portfolio_audit_events(self, run_id: str, events: list[PortfolioAuditEvent]) -> None:
        ...
        for event in ordered:
            event_id = audit_event_id(event)
            if event_id in seen or event.strategy_id not in members: raise ValueError("audit events must be unique and reference persisted portfolio members")
            seen.add(event_id); rows.append((run_id, event_id, event.sequence, event.strategy_id, event.action, event.timestamp, event.quantity, event.price, event.fee))
        with self._connection: self._connection.executemany("INSERT INTO portfolio_audit_events(run_id, event_id, sequence, strategy_id, action, timestamp, quantity, price, fee) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)", rows)
    ...
    def save_reproducibility_certificate(self, certificate: Any) -> None:
        run_id = str(certificate.run_id); certificate_id = str(certificate.certificate_id)
        if not run_id or not certificate_id: raise ValueError("certificate run_id and certificate_id are required")
        if self.get_portfolio_run(run_id) is None: raise ValueError(f"unknown portfolio run: {run_id}")
        ...
        if existing is not None:
            if existing["certificate_id"] != certificate_id or existing["certificate_json"] != payload: raise ValueError("reproducibility certificate already exists and is immutable")
            return
