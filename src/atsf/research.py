...
def _build_research_portfolio(result: GenerationResult, dataset_id: str, dataset_version: str, source_data: pd.DataFrame, store: ExperimentRegistry, *, source: str, timeframe: str, schema_version: str) -> tuple[str | None, PortfolioFeedbackResult | None]:
    ...
    if not return_series:
        return None, None
    returns = pd.concat(return_series, axis=1, join="inner").sort_index(); returns.columns = list(return_series)
    selection_policy = PortfolioPolicy()
    if len(returns) < selection_policy.min_history:
        return None, None
    ranked_inputs = [(evaluation.candidate_id, FitnessResult(score=evaluation.fitness.score, eligible=True, reasons=evaluation.promotion.reasons), 1.0 if evaluation.robustness.passed else 0.0, 0.0) for evaluation in eligible]
    ranked = rank_candidates(ranked_inputs)
    intelligence = build_portfolio_intelligence(ranked, returns)
    feedback = build_portfolio_feedback(intelligence, generation=result.generation)
    if not intelligence.admission_passed:
    ...
    definition = {"dataset_id": dataset_id, "dataset_version": dataset_version, "data_bundle_version": bundle.version, "data_source": source, "data_timeframe": timeframe, "data_schema_version": schema_version, "generation": result.generation, "experiment_ids": {evaluation.candidate_id: evaluation.experiment.experiment_id for evaluation in eligible}, "selection_policy": {"max_strategies": selection_policy.max_strategies, "max_average_correlation": selection_policy.max_average_correlation, "min_history": selection_policy.min_history}, "allocation_policy": {"max_total_weight": allocation_policy.max_total_weight, "min_weight": allocation_policy.min_weight, "max_weight": allocation_policy.max_weight, "volatility_floor": allocation_policy.volatility_floor}, "ranked": [{"strategy_id": item.candidate_id, "fitness_score": item.fitness_score, "robustness_score": item.robustness_score, "diversity_score": item.diversity_score, "final_score": item.final_score} for item in portfolio.ranked], "selected": list(portfolio.selection.selected), "rejected": list(portfolio.selection.rejected), "average_correlation": portfolio.selection.average_correlation, "weights": portfolio.allocation.weights, "estimated_volatility": portfolio.allocation.estimated_volatility, "portfolio_volatility": portfolio.portfolio_volatility, "portfolio_admission_passed": portfolio.admission_passed, "rejected_for_risk": list(portfolio.rejected_for_risk), "total_weight": portfolio.allocation.total_weight}
    canonical = json.dumps(definition, sort_keys=True, allow_nan=False, separators=(",", ":")); portfolio_id = hashlib.sha256(canonical.encode("utf-8")).hexdigest()[:16]
    store.save_portfolio(portfolio_id, definition, portfolio.allocation.weights)
    return portfolio_id, feedback


def _cycle_payload(plan: ResearchPlan, feedback: ResearchFeedbackResult, admissions: tuple[SuccessorAdmission, ...], portfolio_feedback: PortfolioFeedbackResult | None) -> tuple[str, dict, dict, list[dict], dict | None]:
...
def run_research(seeds: list[StrategySpec], data: pd.DataFrame, *, generations: int = 1, population_size: int = 10, survivor_count: int = 3, seed: int = 0, dataset_id: str = "research", registry: ExperimentRegistry | None = None, fitness_policy: FitnessPolicy | None = None, director_policy: ResearchDirectorPolicy | None = None, budget_policy: ResearchBudgetPolicy | None = None) -> ResearchRunResult:
    ...
    frame_identity = dataset_identity(data, dataset_id); population = seed_population(seeds)
    ...
    owned_registry = registry is None; store = registry or ExperimentRegistry(); cycle_store = ResearchCycleRegistry(store._connection); lifecycle_store = LifecycleStore(store._connection)
    results: list[GenerationResult] = []; research_plans: list[ResearchPlan] = []; research_feedback: list[ResearchFeedbackResult] = []; portfolio_feedbacks: list[PortfolioFeedbackResult] = []; portfolio_id: str | None = None
    try:
        ...
        for generation in range(generations):
            ...
            result = evolve_generation(population, data, registered_identity.dataset_id, registered_identity.version, target_size=population_size, survivor_count=survivor_count, seed=seed + generation, fitness_policy=fitness_policy); results.append(result)
            ...
            portfolio_id_for_generation, portfolio_feedback = _build_research_portfolio(result, registered_identity.dataset_id, registered_identity.version, data, store, source=source, timeframe=timeframe, schema_version=schema_version)
            if portfolio_feedback is not None: portfolio_feedbacks.append(portfolio_feedback)
            if portfolio_id_for_generation is not None: portfolio_id = portfolio_id_for_generation
            additional_signals = () if portfolio_feedback is None else portfolio_feedback.signals
            plan = build_research_plan(result, director_policy=director_policy, budget_policy=budget_policy, population=population, additional_signals=additional_signals); research_plans.append(plan)
            ...
            cycle_id, plan_payload, feedback_payload, admissions_payload, portfolio_payload = _cycle_payload(plan, feedback, tuple(admissions), portfolio_feedback)
            cycle_store.save_cycle(cycle_id, result.generation, plan=plan_payload, feedback=feedback_payload, admissions=admissions_payload, portfolio_feedback=portfolio_payload)
            for evaluation in result.evaluations:
                candidate = candidates_by_id[evaluation.candidate_id]; spec = ExperimentSpec(candidate.strategy, registered_identity.dataset_id, registered_identity.version, seed + generation); store.save_experiment(spec, evaluation.experiment)
                store.save_evaluation_evidence(evaluation.experiment.experiment_id, {"candidate_id": evaluation.candidate_id, "walk_forward": {"passed": evaluation.walk_forward.passed, "oos_return": evaluation.walk_forward.oos_return, "oos_sharpe": evaluation.walk_forward.oos_sharpe, "oos_drawdown": evaluation.walk_forward.oos_drawdown}, "monte_carlo": {"simulations": evaluation.monte_carlo.simulations, "seed": evaluation.monte_carlo.seed, "median_return": evaluation.monte_carlo.median_return, "worst_return": evaluation.monte_carlo.worst_return, "lower_percentile_return": evaluation.monte_carlo.lower_percentile_return, "pass_rate": evaluation.monte_carlo.pass_rate}, "perturbation": {"samples": evaluation.perturbation.samples, "seed": evaluation.perturbation.seed, "pass_rate": evaluation.perturbation.pass_rate, "worst_score": evaluation.perturbation.worst_score, "median_score": evaluation.perturbation.median_score, "strategy_ids": evaluation.perturbation.strategy_ids}, "regime": {"score": evaluation.regime.score, "regime_returns": evaluation.regime.regime_returns, "covered_regimes": evaluation.regime.covered_regimes}, "robustness": {"passed": evaluation.robustness.passed, "reasons": evaluation.robustness.reasons, "scenarios": {name: {"total_return": scenario.total_return, "max_drawdown": scenario.max_drawdown} for name, scenario in evaluation.robustness.scenarios}}, "promotion": {"stage": evaluation.promotion.stage, "eligible": evaluation.promotion.eligible, "reasons": evaluation.promotion.reasons}, "research_plan": {"generation": plan.generation, "requests": [request.request_id for request in plan.requests], "allocations": {allocation.request_id: allocation.units for allocation in plan.allocations}}, "research_feedback": {"generation": feedback.generation, "signals": [{"strategy_id": signal.strategy_id, "reason": signal.reason, "fitness": signal.fitness, "robustness": signal.robustness, "novelty": signal.novelty, "uncertainty": signal.uncertainty, "capacity_gap": signal.capacity_gap} for signal in feedback.signals if signal.strategy_id == evaluation.candidate_id]}, "portfolio_feedback": None if portfolio_feedback is None else {"generation": portfolio_feedback.generation, "portfolio_volatility": portfolio_feedback.portfolio_volatility, "admission_passed": portfolio_feedback.admission_passed, "average_correlation": portfolio_feedback.average_correlation, "signals": [{"strategy_id": signal.strategy_id, "reason": signal.reason, "fitness": signal.fitness, "robustness": signal.robustness, "novelty": signal.novelty, "uncertainty": signal.uncertainty, "capacity_gap": signal.capacity_gap} for signal in portfolio_feedback.signals if signal.strategy_id == evaluation.candidate_id]}, "successor_admission": {"cycle_id": cycle_id, "admitted": next((admission.admitted for admission in admissions if admission.candidate_id == evaluation.candidate_id), False)}})
                synchronize_candidate_lifecycle(lifecycle_store, evaluation)
