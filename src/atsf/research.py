...
def _build_research_portfolio(result: GenerationResult, dataset_id: str, dataset_version: str, source_data: pd.DataFrame, store: ExperimentRegistry, *, source: str, timeframe: str, schema_version: str) -> str | None:
    ...
    if not return_series: return None
    ...
    if len(returns) < selection_policy.min_history: return None
...
def _cycle_payload(plan: ResearchPlan, feedback: ResearchFeedbackResult, admissions: tuple[SuccessorAdmission, ...]) -> tuple[str, dict, dict, list[dict]]:
    ...
    canonical = json.dumps({"plan": plan_payload, "feedback": feedback_payload, "admissions": admissions_payload}, sort_keys=True, allow_nan=False, separators=(",", ":"))
    return hashlib.sha256(canonical.encode("utf-8")).hexdigest()[:16], plan_payload, feedback_payload, admissions_payload
...
def run_research(seeds: list[StrategySpec], data: pd.DataFrame, *, generations: int = 1, population_size: int = 10, survivor_count: int = 3, seed: int = 0, dataset_id: str = "research", registry: ExperimentRegistry | None = None, fitness_policy: FitnessPolicy | None = None, director_policy: ResearchDirectorPolicy | None = None, budget_policy: ResearchBudgetPolicy | None = None) -> ResearchRunResult:
    ...
    results: list[GenerationResult] = []; research_plans: list[ResearchPlan] = []; research_feedback: list[ResearchFeedbackResult] = []; portfolio_id: str | None = None
    try:
        ...
        for generation in range(generations):
            ...
            cycle_id, plan_payload, feedback_payload, admissions_payload = _cycle_payload(plan, feedback, tuple(admissions))
            ...
            portfolio_id = _build_research_portfolio(result, registered_identity.dataset_id, registered_identity.version, data, store, source=source, timeframe=timeframe, schema_version=schema_version) or portfolio_id
    ...
    return ResearchRunResult(generations=tuple(results), final_population=tuple(population), dataset_id=frame_identity.dataset_id, dataset_version=registered_identity.version, portfolio_id=portfolio_id, research_plans=tuple(research_plans), research_feedback=tuple(research_feedback))
