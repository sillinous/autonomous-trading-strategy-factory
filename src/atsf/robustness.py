...
def monte_carlo_trade_bootstrap(
    ...
    """Bootstrap trade outcomes to estimate path-independent outcome risk."""
    if simulations <= 0:
    ...
    return MonteCarloResult(
        simulations,
        seed,
        float(np.median(outcomes)),
        float(np.min(outcomes)),
...
def default_scenarios(config: BacktestConfig | None = None) -> tuple[RobustnessScenario, ...]:
    ...
    return (
        ...
        RobustnessScenario(
            ...
            replace(
                base,
                commission_bps=base.commission_bps * 2,
...
def test_robustness(
    ...
    """Run execution-cost stress scenarios using the complete entry/exit lifecycle."""
    ...
    position = _position_signal(entry, exit_)
    results = tuple(
        (
            scenario.name,
            run_long_signal_backtest(data, position, candidate.strategy, scenario.config),
        )
...
def score_regime_stability(regimes: dict[str, pd.Series]) -> RegimeStabilityResult:
    ...
    # Zero means equal average returns across covered regimes; negative values
    # measure how far the weakest regime trails the strongest.
